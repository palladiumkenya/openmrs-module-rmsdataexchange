 ...
 */
package org.openmrs.module.rmsdataexchange.chore;
...
public class SyncPatientsToRMS {
	...
	public void perform(PrintWriter output) {
		...
		if (AdviceUtils.isRMSIntegrationEnabled() && !AdviceUtils.getRMSSyncStatus()) {
			if (debugMode)
				System.err.println("Cashier RMS Patients Sync: Starting ...");
	...
	private SimpleObject patientsSync(PrintWriter output) {
		...
		try {
			...
			if (debugMode)
				System.err.println("Cashier RMS Patients Sync: ERROR: " + ex.getMessage());
			if (debugMode)
				display.println("Cashier RMS Patients Sync: ERROR: " + ex.getMessage());
			if (debugMode)
				ex.printStackTrace();
	...
	private class runTask implements Runnable {
		...
		public void run() {
			// Run the task
			if (debugMode)
				System.out.println("Cashier RMS Patients Sync: Starting the SyncPatientsToRMS chore");
			if (debugMode)
				output.println("Cashier RMS Patients Sync: Starting the SyncPatientsToRMS chore");
			...
			try {
				...
				if (debugMode)
					System.out.println("Cashier RMS Patients Sync: Completed syncing patients to RMS");
				if (debugMode)
					output.println("Cashier RMS Patients Sync: Completed  syncing patients to RMS");
			}
			catch (Exception ex) {
				if (debugMode)
					System.err.println("Cashier RMS Patients Sync: ERROR: SyncPatientsToRMS chore: " + ex.getMessage());
				if (debugMode)
					output.println("Cashier RMS Patients Sync: ERROR: SyncPatientsToRMS chore: " + ex.getMessage());
				if (debugMode)
					ex.printStackTrace();
			}
		}
