...
public class NewBillPaymentSyncToRMS implements MethodInterceptor {
	...
	private class syncPaymentRunnable implements Runnable {
		...
		public void run() {
			...
			try {
				if (debugMode)
					System.out.println("RMS Sync Cashier Module: Start sending payment to RMS");
				
				// If the patient doesnt exist, send the patient to RMS
				if (debugMode)
					System.out.println("RMS Sync Cashier Module Bill Payment: Send the patient first");
				...
				if (debugMode)
					System.out.println("RMS Sync Cashier Module Bill Payment: Send the bill next");
				...
				if (debugMode)
					System.out.println("RMS Sync Cashier Module: Finished sending payment to RMS");
			}
			catch (Exception ex) {
				if (debugMode)
					System.err.println("RMS Sync Cashier Module: Error. Failed to send payment to RMS: " + ex.getMessage());
				ex.printStackTrace();
			}
		}
	}
