...
public class SyncBillsToRMS {
	...
	public void perform(PrintWriter output) {
		...
		if (AdviceUtils.isRMSIntegrationEnabled() && !AdviceUtils.getRMSSyncStatus()) {
			if (debugMode)
				System.err.println("Cashier RMS Bills Sync: Starting ...");
	...
	private SimpleObject billsSync(PrintWriter output) {
		...
		try {
			...
			if (debugMode)
				System.err.println("Cashier RMS Bills Sync: ERROR: " + ex.getMessage());
			if (debugMode)
				display.println("Cashier RMS Bills Sync: ERROR: " + ex.getMessage());
			if (debugMode)
				ex.printStackTrace();
	...
	private class runTask implements Runnable {
		...
		public void run() {
			// Run the task
			if (debugMode)
				System.out.println("Cashier RMS Bills Sync: Starting the SyncBillsToRMS chore");
			if (debugMode)
				output.println("Cashier RMS Bills Sync: Starting the SyncBillsToRMS chore");
			...
			try {
				...
				if (debugMode)
					System.out.println("Cashier RMS Bills Sync: Completed syncing patients to RMS");
				if (debugMode)
					output.println("Cashier RMS Bills Sync: Completed  syncing patients to RMS");
			}
			catch (Exception ex) {
				if (debugMode)
					System.err.println("Cashier RMS Bills Sync: ERROR: SyncBillsToRMS chore: " + ex.getMessage());
				if (debugMode)
					output.println("Cashier RMS Bills Sync: ERROR: SyncBillsToRMS chore: " + ex.getMessage());
				if (debugMode)
					ex.printStackTrace();
			}
		}
